        ...
        <div class="relative w-full max-w-xl bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col">
          <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            <div>
              ...
              <h3 class="text-sm font-normal text-slate-400">{{ id }}</h3>
            </div>
          ...
          <div class="px-6 py-4 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3">
            ...
            <button 
              @click="emit('confirm')" 
              :disabled="loading"
              class="px-6 py-2 text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50 rounded-xl shadow-lg shadow-blue-600/20 transition-all flex items-center gap-2"
            >
              <span v-if="loading" class="text-lg">
                <Loader :size="17"  class="text-white animate-spin "/>
              </span>
              {{ confirmLabel || 'Simpan' }}
            </button>
          </div>
