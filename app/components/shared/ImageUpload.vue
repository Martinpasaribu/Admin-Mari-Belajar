...
<template>
  <div class="flex flex-col gap-2 w-full">
    ...
    <div v-if="multiple" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
      ...
      <label class="aspect-square border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:border-blue-400 hover:bg-blue-50/50 transition-all group">
        <div class="w-8 h-8 bg-white border border-slate-100 shadow-sm rounded-lg flex items-center justify-center mb-1 group-hover:scale-110 transition-transform text-slate-400">＋</div>
    ...
    <div 
      v-else
      :class="[
        'relative group border-2 border-dashed rounded-2xl transition-all duration-300 flex flex-col items-center justify-center overflow-hidden bg-slate-50/50',
    ...
    >
      ...
      <label v-else class="w-full h-full flex flex-col items-center justify-center cursor-pointer p-4">
        <div class="w-10 h-10 bg-white border border-slate-100 shadow-sm rounded-xl flex items-center justify-center mb-2 group-hover:scale-110 transition-transform">
