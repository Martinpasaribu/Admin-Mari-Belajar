...
// 1. Buat fungsi helper untuk nilai awal agar konsisten
const getInitialForm = () => ({
  bab_key: props.bab?._id || "",
...
const handleKeyDown = (e: KeyboardEvent) => {
  ...
  // Jika tekan Escape -> Tutup Modal
  if (e.key === 'Escape') {
    emit('close');
  }

  if (e.key === 'Enter' && e.target?.tagName !== 'TEXTAREA') {
    if (!isSubmitting.value) { // Gunakan state isSubmitting lokal
      handleSave(); // Panggil fungsi save lokal, bukan emit
    }
  }
};
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div class="p-4 border border-dashed border-slate-300 rounded-2xl">
                ...
                <div class="mt-8">
                    ...
                    <ImageUpload v-model="form.question_images" multiple label="Tambah Gambar" />
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="lg:col-span-5 space-y-4 lg:sticky lg:top-0">
          ...
          <div v-if="form.type !== 'essay'" class="space-y-4">
            ...
            <div class="mt-4 p-4 bg-emerald-50 rounded-xl border border-emerald-100 shadow-sm">
          ...
          <div v-else class="p-5 bg-emerald-50 rounded-2xl border border-emerald-100 space-y-3 shadow-sm">
            <label :class="labelClass" class="text-emerald-700">Kunci Jawaban (Essay)</label>
            <textarea v-model="form.correct_answer" rows="6" :class="inputClass" placeholder="Masukkan kata kunci atau jawaban benar..."></textarea>
          </div>
