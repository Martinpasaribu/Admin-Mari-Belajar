// server/api/sub-categories/index.ts
...
export default defineEventHandler(async (event) => {
  ...
  try {
    if (method === 'GET') {
      ...
      const res = await http.get('/sub-categories', {
        headers: { 
          cookie: cookie || '' 
        }
      })
    ...
    if (method === 'POST') {
      ...
      const res = await http.post('/sub-categories', body, {
        headers: { 
          cookie: cookie || '' 
        }
      })
    ...
    console.error(`❌ Error ${method} /sub-categories:`, err.response?.data || err.message)
